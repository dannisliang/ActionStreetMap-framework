...
namespace Mercraft.Infrastructure.Threading
{
    public static class EnumerableExtension
    {
        ...
        {
            return (IEnumerable<Task>)that.ParallelForEach(element => { action(element); return default(Mercraft.Infrastructure.Threading.Task.Unit); }, target);
        }
        ...
        public static IEnumerable<Task<TResult>> ParallelForEach<TResult, T>(this IEnumerable<T> that, Func<T, TResult> action, TaskDistributor target)
        {
            ...
            {
                var tmp = element;
                var task = Task.Factory.Create(() => action(tmp)).Run(target);
                result.Add(task);
            }
            return result;
        }
        ...
        {
            return (IEnumerable<Task>)that.SequentialForEach(element => { action(element); return default(Mercraft.Infrastructure.Threading.Task.Unit); }, target);
        }
        ...
        public static IEnumerable<Task<TResult>> SequentialForEach<TResult, T>(this IEnumerable<T> that, Func<T, TResult> action, TaskDistributor target)
        {
            ...
            {
                var tmp = element;
                var task = Task.Factory.Create(() => action(tmp));
                if (lastTask == null)
                    task.Run(target);
                else
