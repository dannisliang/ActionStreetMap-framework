...
namespace Mercraft.Infrastructure.Threading
{
    public abstract class DispatcherBase : IDisposable
    {
        ...
        public void Unlock()
        {
            lock (taskListSyncRoot)
            {
                ...
                if (lockCount == 0 && delayedTaskList.Count > 0)
                {
                    ...
                    if (TaskSortingSystem == Mercraft.Infrastructure.Threading.TaskSortingSystem.ReorderWhenAdded ||
                    TaskSortingSystem == Mercraft.Infrastructure.Threading.TaskSortingSystem.ReorderWhenExecuted)
                        ReorderTasks();
        ...
        public Task Dispatch(Action action)
        {
            ...
            var task = Task.Factory.Create(action);
            AddTask(task);
            return task;
        }
        ...
        internal void AddTasks(IEnumerable<Task> tasks)
        {
            lock (taskListSyncRoot)
            {
                ...
                if (TaskSortingSystem == Mercraft.Infrastructure.Threading.TaskSortingSystem.ReorderWhenAdded || TaskSortingSystem == Mercraft.Infrastructure.Threading.TaskSortingSystem.ReorderWhenExecuted)
					 ReorderTasks();
            }
			TasksAdded();
        }
